...
export default function SimulationGameScreen({navigation, route}) {
  ...
  console.log('🔍 tradingInterval:', config?.tradingInterval);
  ...
  // 🆕 동적 totalSteps 계산 (기존 라인 교체)
  const [totalSteps] = useState(() => {
    const interval = config?.tradingInterval || 'monthly';

    console.log('🔍 interval 계산 결과:', interval); // 🔍 추가 로그

    if (interval === 'quarterly') {
      console.log('✅ 분기별 모드: 8단계'); // 🔍 추가 로그
      return 8; // 2년을 8분기로
    } else if (interval === 'monthly') {
      console.log('✅ 월별 모드: 24단계'); // 🔍 추가 로그
      return 24; // 2년을 24개월로
    }
    console.log('⚠️ 기본값 사용: 24단계'); // 🔍 추가 로그
    return parseInt(config?.totalSteps || '24'); // 기존 로직 유지
  });

  const [currentDate, setCurrentDate] = useState(config?.startDate || '2023-01-01');
  // 🆕 진행률 정보 계산 함수 추가 (나중에 사용)
  const getProgressInfo = () => {
    const progress = (currentStep / totalSteps) * 100;
    const periodText = config?.tradingInterval === 'quarterly' ? '분기' : '개월';
    const modeText = config?.tradingInterval === 'quarterly' ? '분기별' : '월별';

    return {progress, periodText, modeText};
  };
...
// 다음 단계로 진행 (성과 평가 포함)
  const nextStep = () => {
    if (currentStep >= totalSteps) {
      ...
      // 🆕 분기별 vs 월별에 따른 다른 완료 메시지
      if (config?.tradingInterval === 'quarterly') {
        // 🎯 분기별 완료 시 업그레이드 유도
        Alert.alert(
            '🎉 분기별 시뮬레이션 완료!',
            `${grade.emoji} 8분기 시뮬레이션을 완료했습니다!
...
더 자세한 월별 시뮬레이션을 해보시겠어요?
월별로 하면 더 정교한 투자 전략을 세울 수 있습니다! 🚀`,
            [
              {
                text: '📊 성적표 보기',
              ...
              },
              {
                ...
                onPress: () => navigation.navigate('SimulationSetup', {
                  upgradeToMonthly: true,
                  previousResults: metrics
                })
              },
              {text: '🔄 분기별 다시', onPress: () => navigation.navigate('SimulationSetup')},
...
정말 대단합니다! 장기 투자 마스터가 되셨네요! 🎯`,
    ...
    // 🆕 분기별에서는 중간 평가 주기 조정
    const evaluationInterval = config?.tradingInterval === 'quarterly' ? 2 : 6; // 분기별은 2분기마다, 월별은 6개월마다

    if (currentStep % evaluationInterval === 0) {
      const metrics = calculatePerformanceMetrics();
      const grade = getInvestmentGrade(metrics.returnPercentage);
      const {periodText} = getProgressInfo();

      Alert.alert(
          '📊 중간 성과 평가',
          `${grade.emoji} ${currentStep}${periodText} 차 성과:
...
계속 화이팅하세요! 🚀`,
      ...
      const {periodText} = getProgressInfo();
      Alert.alert(`📅 다음 ${periodText}`, `${periodText} 단계가 진행되었습니다!\n현재: ${currentStep + 1}/${totalSteps} ${periodText}`);
    }
  };
  ...
  const {modeText} = getProgressInfo();
  ...
  return (
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>🎮 {modeText} 시뮬레이션</Text>
          <TouchableOpacity
        ...
        {/* 🆕 진행률 바 추가 (statusBar 위에) */}
        <View style={styles.progressContainer}>
          <View style={[styles.progressBar, {width: `${getProgressInfo().progress}%`}]}/>
          <Text style={styles.progressText}>
            {getProgressInfo().progress.toFixed(0)}% 완료
          </Text>
        </View>
        ...
        <View style={styles.statusBar}>
          <Text style={styles.stepText}>
            📅 {currentDate} ({currentStep}/{totalSteps} {getProgressInfo().periodText})
          </Text>
        ...
        {/* 🆕 힌트 카드 추가 (portfolioSummary 다음에) */}
        {config?.tradingInterval === 'quarterly' && currentStep <= 2 && (
            <View style={styles.hintCard}>
              <Text style={styles.hintText}>
                💡 분기별 모드는 빠르게 체험해볼 수 있어요!{'\n'}
                재미있다면 월별 모드도 도전해보세요! 🚀
              </Text>
            </View>
        )}
        ...
        <View style={styles.actionButtons}>
          ...
          >
            <Text style={styles.nextButtonText}>
              {currentStep >= totalSteps ? '🎉 완료!' : `📅 다음 ${getProgressInfo().periodText}`}
            </Text>
          </TouchableOpacity>
        </View>
    </View>  // ← 이게 누락됨!
